...
class MPTokopediaProduct(models.Model):
    _name = 'mp.tokopedia.product'
    _inherit = 'mp.bigint'
    _description = 'Tokopedia Product'

    name = fields.Char()
    active = fields.Boolean()
    mp_product_wsl_ids = fields.One2many(
        'mp.tokopedia.product.wsl', 'mp_product_id', string='Wholesale')
...
class MPTokopediaCategoryVariant(models.Model):
    ...
    units = fields.Many2many(
        ...
        'mp_tokopedia_category_variant_id',
        'mp_tokopedia_category_unit_id')
...
class MPTokopediaShop(models.Model):
    ...
    # slash_price_ids = fields.One2many('mp.tokopedia.product.slash.price', 'shop_id')

    # mp_product_ids = fields.One2many('mp.tokopedia.product', 'shop_id', string='Product')
...
class MPTokopediaProductWsl(models.Model):
    ...
    min_qty = BigInteger()
    price = BigInteger()
    mp_product_id = BigMany2one(
        'mp.tokopedia.product', string='Product', ondelete='cascade')
    active = fields.Boolean(default=True)
    izi_id = fields.Integer('Izi ID')
    izi_md5 = fields.Char()
