...
class WebhookServer(models.Model):
    ...
    def delete_data(self):
        ...
        sm_ids = sm.ids
        sq_ids = self.env['stock.quant'].sudo().search([('product_id', 'in', pp_ids)]).ids
    ...
    def remote(self):
        ...
        if self.username and self.password:
            dbname = self.name if not res else res.get('response', {}).get('dbname')
        ...
        if access_token:
            return {                   
                ...
                'target': 'new',
                'url': '%s/web/login/token/%s' % (self.name, access_token),
            }
        else:
            return {}
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    return {                   
                        ...
                        'type': 'ir.actions.act_url',
                        'target': 'new',
                        'url': '%s/web/login/rsa/%s' % (self.name, access_token),
                    }
                else:
