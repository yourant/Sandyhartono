...
class OauthAccessToken(models.Model):
    ...
    def _get_access_token(self, user_id=None, create=False):
        ...
        if not access_token and create:
            expires = datetime.now() + timedelta(seconds=int(self.env.ref('juragan_webhook.oauth2_access_token_expires_in').sudo().value))
            vals = {
                'user_id': user_id,
                'scope': 'userinfo',
                'expires': expires.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                'token': oauthlib_common.generate_token(length=200),
            }
...
class Users(models.Model):
    ...
    token_ids = fields.One2many(
        'oauth.access_token', 'user_id', string="Access Tokens")
